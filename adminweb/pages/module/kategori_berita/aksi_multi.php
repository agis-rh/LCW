...
Tidak ada data kategori berita yang dipilih untuk dihapus.</div>
<a href='javascript:;' onclick='self.history.back()' class='button round dark' />Kembali</a>";
    ...
    if(count($cekid) < 1) {
        echo "<br><div class='error-box round'>
Tidak ada data kategori berita yang dipilih untuk diedit.</div>
<a href='javascript:;' onclick='self.history.back()' class='button round dark' />Kembali</a>";
        ...
        $cekid  =   $_POST['cek'];
            for($i=0;$i<count($cekid);$i++) {
					...
					<div class="content-module-main">
                                            <?php
                                            $data   = $fquery->get_one_kategori($cekid[$i]);
                                            ?>
                                                <fieldset>
									<p>
										<label for="full-width-input">Nama kategori Profesi</label>
                                                                                ...
                                                                                <input type="text" value="<?php echo $data['nama']; ?>" required placeholder="Kategori berita . . . . ." name="nama[]" id="full-width-input" class="default-width-input form-control"/>
...
if($_POST['save']) {
    ...
    if($jumlah < 1) {
        ...
        for($ai=1;$ai<=$jumlah;$ai++) {
            ...
            $fquery->edit_kategori($nama[$a],$ket[$a],$idkategori[$a]);
            echo "<script>window.location='admin.php?page=kategori_berita';</script>";
        }
    }
}
    ...
    $fquery->add_log_team($id_team,"menambahkan $jumlah_add kategori berita",$tanggal,$jam);

    for($b=1;$b<=$jumlah_add;$b++) {
        $c = $b-1;
            $fquery->add_kategori($judul[$c],$konten[$c]);
            echo "<script>window.location='admin.php?page=kategori_berita';</script>";
    }
}